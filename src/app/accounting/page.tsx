...
export default function AccountingPage() {
  ...
  const [purchases, setPurchases] = useState<{ id: string; supplier?: string; total_amount: number; order_date: string; status?: string }[]>([]);
  ...
  async function loadAll() {
    ...
    const { data: pur } = await supabase.from('purchase_orders').select('id, total_amount, order_date, status').gte('order_date', from).lte('order_date', to).order('order_date', { ascending: false });
    setPurchases(pur || []);
  }
      ...
      {tab === 'purchases' && (
        <div>
          ...
          <div className="bg-white border rounded-2xl overflow-hidden shadow-sm">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b"><tr>
                {['Date', 'Total', 'Status'].map((h) => <th key={h} className="px-4 py-3 text-left text-xs text-gray-500 font-medium">{h}</th>)}
              </tr></thead>
              <tbody className="divide-y">
                {purchases.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3"><span className="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-700">{p.status || 'ordered'}</span></td>
                  </tr>
                ))}
                {purchases.length === 0 && <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400">No purchases</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
